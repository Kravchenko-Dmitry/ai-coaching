...
## (Optional) Solution assessment against the initial assignment

This prototype was built against four framing questions from the exercise brief. Each is
addressed below along with the simplifications made and the risk each one carries.
...
per-repo via `.mcp.json`. Every developer's machine needs the .NET 8 SDK and a checked-out copy of
this repo so `.mcp.json` can point `dotnet run --project`. For a real team the REST API would need to
run as a shared, always-on service with real auth turned on (`Security:ApiKey` exists but is
disabled by default). The architect-facing Q&A channel is REST-only — no UI, that means either of them
curl/Postman/a script shall be executed.
...
did it. Both are reasonable MVP simplifications, but a production version
